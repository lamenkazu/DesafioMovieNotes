...
class NotesController{
    ...
    async create(req, res){
        ...
        if(!user) throw new AppError('Nao é possível criar notas para um usuário que não existe.')
    ...
    async update(req, res){
        ...
        const note = await knex('notes').where({note_id}).first()
        if(!note) throw new AppError('Essa nota não existe, então não pode ser atualizada.')
    ...
    async delete(req, res){
        ...
        const note = await knex('notes').where({note_id}).first()
        if(!note) throw new AppError('Essa nota não existe, então não pode ser deletada.')
